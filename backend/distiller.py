...
def get_explanation_prompt(level: ExplanationLevel) -> str:
    ...
    prompts = {
        ExplanationLevel.FIVE_YEAR_OLD: (
            "AUDIENCE: a complete beginner. Use very simple words, everyday analogies, "
            "and zero jargon. Define every term you introduce and keep sentences short."
        ),
        ExplanationLevel.INTERN: (
            "AUDIENCE: an INTERMEDIATE learner who already knows the fundamentals. "
            "Do NOT re-explain the basics and do NOT talk down to them. Use correct "
            "technical terminology (briefly defining only advanced terms), give concrete "
            "worked examples, and always explain the 'why' and the trade-offs behind concepts."
        ),
        ExplanationLevel.SENIOR: (
            "AUDIENCE: an ADVANCED/senior professional. Be precise and information-dense, "
            "use domain terminology freely, and focus on trade-offs, edge cases, internals, "
            "and best practices. Skip all introductory explanation."
        ),
    }
...
async def gen_flashcards_quiz(
    ...
    *,
    ...
    """Generate flashcards and quiz grounded in retrieval context for a topic."""
    explanation_prompt = get_explanation_prompt(explanation_level)
    topic_label = (topic or summary or "the study topic").strip()
    if topic_label.lower().startswith("topic:"):
        topic_label = topic_label.split(":", 1)[1].strip() or topic_label
    ground = (retrieval_context or "").strip()
    try:
    ...
    prompt = (
        f"Create high-quality learning materials about: {topic_label}\n"
        f"{explanation_prompt}\n\n"
        "GROUND TRUTH (base every flashcard and quiz question ONLY on this content — "
        "do not invent facts not supported here):\n"
        f"\"\"\"\n{ground or '(limited source — use accurate, realistic content about ' + topic_label + ')'}\n\"\"\"\n\n"
    ...
    try:
        ctx = ground[:4000] if len(ground) > 4000 else ground
        messages = [{"role": "user", "content": prompt.replace(ground, ctx) if ground else prompt}]
        ...
        if isinstance(data, dict) and ("flashcards" in data and "quiz" in data):
            return data
        return _get_fallback_flashcards_quiz(topic_label, n_items)
    except Exception as e:
        logger.error(f"LLM flashcards/quiz failed: {e}")
        return _get_fallback_flashcards_quiz(topic_label, n_items)

def _get_fallback_flashcards_quiz(topic: str, num_items: int = 5) -> Dict[str, list]:
    """Generate fallback flashcards and quiz when LLM fails."""
    words = topic.split()
    key_concepts = [word for word in words if len(word) > 3][:max(1, num_items)]
    if not key_concepts:
        key_concepts = [topic[:40] or "concept"]

    flashcards = []
    for concept in key_concepts:
        flashcards.append({
            "front": f"What is {concept} in the context of {topic}?",
            "back": f"{concept} is a key idea related to {topic}.",
        })

    quiz = []
    for i, concept in enumerate(key_concepts):
        quiz.append({
            "question": f"Which statement best relates to {concept} in {topic}?",
...
def _apply_study_focus_topic(extracted: str, study_focus: Optional[str], default: str) -> str:
    """Return the active study topic.

    The Learn-page Apply button sends ``study_focus`` — that always wins so we
    don't accidentally treat the whole quick-action prompt ("Generate quiz for
    the current material…") as the topic string.
    """
    ...
    if (
        ...
        and tl not in bad
        and len(t) < 120
        and not any(m in tl for m in junk_markers)
    ):
        return t
    return default


async def _fetch_kb_grounding(query: str, top_k: int = 6) -> str:
    """Fresh Foundry/Supabase retrieval for a topic — never reuse a stale cache."""
    q = (query or "").strip()
    if not q:
        return ""
    try:
        ...
        logger.debug(f"KB grounding fetch failed: {e}")
    return ""


async def _retrieve_pdf_chunks(conv_id: str, topic: str, top_k: int = 6) -> str:
    """Top PDF chunks for *this* topic (vector search over uploaded doc)."""
    ...
    try:
        ...
        sims = find_similar_content(q_embed, embeds, top_k=top_k)
        top_indices = [i for i, _ in sims]
        top_texts = [chunks[i] for i in top_indices if i < len(chunks)]
        return "\n\n".join(top_texts)[:6000]
    except Exception as e:
...
async def process_chat_message(
    ...
    try:
        ...
        # Get conversation context — file_context is PDF text only (set on upload).
        # KB grounding is fetched fresh per quick-action via _resolve_grounding so
        # changing the Apply topic never blends with a previous topic's cache.
...
async def _handle_lesson_generation(
    ...
    try:
        ...
        plan_prompt = (
            ...
            f"Framework Context: {framework}\n{get_explanation_prompt(explanation_level)}\n\n"
            "GROUND TRUTH (anchor every module to concepts from this material only):\n"
            f"\"\"\"\n{ground or '(limited source — use accurate domain knowledge about ' + topic + ')'}\n\"\"\"\n\n"
            "Return JSON: {\n"
            "  \"title\": str, \"overview\": str, \"difficulty_level\": \"beginner|intermediate|advanced\",\n"
            "  \"learning_topics\": [ { \"topic\": str, \"description\": str, \"estimated_time\": str, \"prerequisites\": [str], \"key_concepts\": [str] } ],\n"
            "  \"learning_path\": [str]\n"
...
Just tell me what you'd like to create!"""
        ...
        help_text = """🤖 **I'm PathWise, your AI learning assistant!**
...
async def _handle_regular_chat(message: str, conv_id: str, file_context: Optional[str], explanation_level: ExplanationLevel) -> Dict:
    ...
    # Build system prompt
    system_prompt = f"""You are PathWise, an AI learning assistant. {get_explanation_prompt(explanation_level)}
...
async def _handle_workflow_generation(
    ...
    try:
        ...
        prompt = (
            ...
            "\"\"\"\n" + (ground or "(no source loaded — produce a credible domain workflow but keep terminology accurate)") + "\n\"\"\"\n\n"
...
async def _handle_summary_generation(
    ...
    try:
        ...
        prompt = f"""
            ...
            GROUND TRUTH (base every bullet ONLY on this content — do not mention an uploaded document unless one exists):
            \"\"\"
            {ground or '(limited source — use accurate knowledge about ' + topic + ')'}
            \"\"\"

            IMPORTANT: Create a brief, focused summary with exactly 10 key bullet points.
            Focus on the most important information about {topic}.
...
async def _handle_diagnostic_generation(
    ...
    try:
        ...
        prompt = (
            ...
            "Ground every question ONLY in the provided source content.\n"
            f"{get_explanation_prompt(explanation_level)}\n\n"
            ...
            "GROUND TRUTH:\n"
            f"{ground or '(limited source — write realistic questions about ' + topic + ')'}\n\n"
            ...
            "      \"explanation\": str         // 1-sentence explanation grounded in the source\n"
            "    }\n"
            "  ]\n"
            "}"
        )
